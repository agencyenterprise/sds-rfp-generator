...
const RFPCard = ({ rfp }: { rfp: RFP }) => (
  <article className="w-[214px] min-w-[255px] shrink-0 grow">
    <a
      href={`/view/${rfp.id}`}
      className="flex h-full items-start gap-2 overflow-hidden rounded-lg border border-slate-800 bg-slate-800 px-4 py-6 shadow-sm"
    >
...
const Board = () => {
  ...
  return (
    <section className="container relative z-10 mx-auto flex flex-col justify-center pb-40 pt-20 max-md:pb-24">
      ...
      <div className="mt-10 flex w-full flex-wrap items-stretch gap-4 px-4 max-md:max-w-full">
        {rfps.map(
          (rfp, index) => index < 4 && <RFPCard key={index} rfp={rfp as RFP} />,
        )}
      </div>
